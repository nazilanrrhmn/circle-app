...
interface ProfileHeadingProps extends UserProfileDTO {
  thumbnailH: string;
  buttonTitle: string;
  isFollow: boolean;
  isMyProfile?: boolean;
...
export default function ProfileHeading({
  ...
  async function handleFollow(followingId: number) {
    try {
      const response = isFollowUser
        ? await apiV1.delete(`/unfollow/${followingId}`)
        : await apiV1.post("/follow", { followingId });

      setIsFollowUser(!isFollowUser);
    ...
    <>
      <Box position="relative" mb={4}>
        ...
        <Flex justifyContent="flex-end" mt={2}>
          <Button
            onClick={isMyProfile ? onOpen : () => handleFollow(id)}
      ...
      <Flex direction="column" gap={2}>
        ...
        <Flex gap={4}>
          <Text fontSize="16px" fontWeight="700" lineHeight="24px">
            {following}{" "}
            <Text as="span" fontWeight="400" color="brand.fontSecondary" ml={1}>
              Following
            </Text>
          </Text>
          <Text fontSize="16px" fontWeight="700" lineHeight="24px">
