...
export default function FormPost({
  ...
  const onImageChage = (
    ...
    if (event.target.files && event.target.files[0]) {
      setImage(URL.createObjectURL(event.target.files[0])); // Tampilkan preview gambar
      onChange(event);
    }
  };

  const handleFormSubmit = async (data: any) => {
    await onSubmit(data); // Tunggu hingga pengiriman selesai
    reset(); // Reset form input values
    setImage(null); // Reset image preview setelah post berhasil
  };
